...
2. GET the KCFinder browse URL with `bros_config` and `brosseccheck` query params to initialize the KCFinder session
...
For standard KCFinder installations where you already have a valid `PHPSESSID` cookie. Use this when KCFinder's session is established by some other mechanism.
...
To support a different authentication scheme, subclass `BaseAuth` and implement its three methods:
